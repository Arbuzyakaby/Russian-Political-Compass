...
for (const id of Object.keys(themeScheme)) test(`тема ${id}: контраст текста и акцента на подложке`, () => {
  ...
  /* Третичный тон проверяется с 2.4.1. До этого он не проверялся вовсе
     и не проходил AA ни в одной из шести тем — от 2.56 в «Аляске» до
     4.05 в тёмной, — а им набрано около сотни мест: подписи плиток,
     вторые строки пунктов, сноски карточек, подвал. Порог тот же 4.5:
     это обычный текст, просто менее важный, а не декорация. */
  assert.ok(contrast(token("--muted-2"), token("--bg")) >= 4.5,
    `${id}: третичный текст не читается на подложке`);
  assert.ok(contrast(token("--muted-2"), token("--bg")) <= contrast(token("--muted"), token("--bg")),
    `${id}: третичный тон стал контрастнее вторичного — иерархия перевернулась`);
});

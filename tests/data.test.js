...
/* ============ ЗАГОТОВКА БУДУЩЕГО СОЗЫВА ============
   Созыв заводится в данных заранее и до выборов помечен draft:true.
   Смысл флага в том, что о заготовке не должен знать никто, кроме
   самих данных: попади она в список созывов — появился бы выбираемый
   созыв, в котором у всех партий ноль мандатов, пустая палата и
   деление на ноль в метриках.

   Поэтому проверяется именно граница: PC.CONVOCATIONS (то, что видит
   интерфейс) заготовку не содержит, PC.CONVOCATIONS_ALL содержит,
   и переход от одного к другому — снятие одного флага. */
test("заготовка созыва существует в данных и не видна интерфейсу", () => {
  const drafts = PC.CONVOCATIONS_ALL.filter(c => c.draft);
  assert.ok(drafts.length >= 1, "заготовки будущего созыва нет — данные к выборам не готовы");

  for(const d of drafts){
    assert.ok(!PC.CONVOCATIONS.some(c => c.id === d.id),
      `созыв ${d.id} помечен draft, но попал в список для интерфейса`);
    assert.ok(!PC.convsAsc().some(c => c.id === d.id),
      `созыв ${d.id} помечен draft, но попал на график динамики`);
    assert.ok(d.id > Math.max(...PC.CONVOCATIONS.map(c => c.id)),
      "заготовка обязана быть новее всех созывов с данными");

    /* Подписи заполнены заранее — в этом и смысл заготовки: после
       выборов правится только флаг и мандаты, а не переводы. */
    assert.ok(String(d.label).trim().length, `у созыва ${d.id} нет русской подписи`);
    assert.ok(d.en && String(d.en.label).trim().length, `у созыва ${d.id} нет английской подписи`);
    assert.match(String(d.years), /^\d{4}[–-]\d{4}$/, `у созыва ${d.id} странные годы: ${d.years}`);

    /* Ни у одной партии не должно быть мандатов в созыве, который
       ещё не состоялся: это данные из будущего. */
    for(const p of PC.PARTIES){
      assert.equal(PC.seatsAt(p, d.id), null,
        `${p.id}: у партии есть мандаты в созыве ${d.id}, которого ещё не было`);
    }
    /* И ни одного голосования: голосовать в несуществующем созыве
       тоже нельзя. */
    assert.ok(!PC.VOTES.some(v => v.conv === d.id),
      `к созыву ${d.id} привязано голосование, а самого созыва ещё нет`);
  }
});

test("текущий созыв — самый новый из тех, по которым есть данные", () => {
  const newest = Math.max(...PC.CONVOCATIONS.map(c => c.id));
  assert.equal(PC.CURRENT_CONVOCATION, newest,
    "умолчание указывает не на самый свежий созыв с данными");
});

test("PC.convocations() и PC.CONVOCATIONS описывают один и тот же набор", () => {
  assert.deepEqual(
    Array.from(PC.convocations(), c => c.id).sort(),
    Array.from(PC.CONVOCATIONS, c => c.id).sort()
  );
});

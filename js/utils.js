...
(function(PC){
  ...
  /* "+3.2" / "-8.0" — знак всегда явный, один знак после запятой.
     Знак ставится по округлённому значению, а не по исходному: иначе
     центр тяжести палаты в 0.04 печатался бы как «+0.0», а в −0.04 —
     как «−0.0», хотя показанное число в обоих случаях ноль. */
  function fmt(v){
    var r = Number(Number(v).toFixed(1));
    if(r === 0) r = 0;                 /* снимает знак у минус-нуля */
  ...
  /* Словарь спрашивается лениво, а не сохраняется в переменную: utils.js
     подключается раньше i18n.js (от него зависят все остальные модули,
     включая сам словарь), но вызывается уже во время отрисовки, когда
     язык определён. Русская ветка остаётся запасной на случай, если
     словарь по какой-то причине не загрузился. */
  function seatsLabel(n){
    if(!n) return PC.t ? PC.t("seats.none") : "нет мандатов";
    return PC.i18n ? PC.i18n.n(n, "word.seat") : n + " мандат" + plural(n);
  }

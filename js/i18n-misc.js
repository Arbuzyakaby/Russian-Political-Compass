...
(function(PC){
  ...
  Object.assign(PC.i18nDict, {
    ...
    "wel.up.l1":          ["Стекло компаса стало заметным: плита светлее, кромка ярче, зазор между линзами шире, тень глубже. В прошлой версии редизайн было почти не видно.",
                           "The compass glass is now actually visible: a lighter plate, a brighter rim, a wider gap between the lenses and a deeper shadow. In the last version the redesign was barely noticeable."],
    "wel.up.l2":          ["Пунктирный поводок к подписи партии больше не висит постоянно — он появляется под курсором. У «Справедливой России» он читался как посторонняя чёрточка на поле.",
                           "The dashed leader to a party label no longer hangs there permanently — it appears under the cursor. On A Just Russia it read as a stray dash on the board."],
    "wel.up.l3":          ["Подсказка, открытая с точки траектории, больше не залипает на экране: слой маршрутов переставлялся посреди наведения и отменял парное событие ухода курсора.",
                           "A tooltip opened from a trajectory node no longer sticks on screen: the trails layer was being reordered mid-hover, which cancelled the matching mouse-leave event."],
    "wel.up.l4":          ["Числа координат у перекрестия больше не ложатся на чужие точки: у самой оси, где им всё равно нечего сказать, они не показываются.",
                           "The crosshair's coordinate numbers no longer land on other parties' dots: right next to an axis, where they have nothing to say anyway, they are simply not drawn."],
    "wel.up.l5":          ["Шаги обучения на приветственном экране стали кликабельными, под ними идёт полоса пройденного, а блок-схема кода в «О проекте» стала крупнее и заметнее.",
                           "The tutorial steps on the welcome screen are now clickable with a progress bar underneath, and the code chain in About has grown bigger and bolder."],
    ...
    "foot.version.tip":   ["Версия 2.4.2: стекло компаса стало заметным, постоянный пунктир у подписей убран, подсказка с траектории больше не залипает, шаги обучения кликабельны, блок-схема в «О проекте» стала крупнее",
                           "Version 2.4.2: the compass glass is now actually visible, the permanent dashed leader is gone, the trajectory tooltip no longer sticks, tutorial steps are clickable, and the code chain in About got bigger"],

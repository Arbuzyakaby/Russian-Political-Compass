...
(function(PC){
  ...
  Object.assign(PC.i18nDict, {
    ...
    "wel.up.l1":          ["Компас переехал на стеклянную подложку: стопка сдвинутых слоёв позади поля убрана, вместо неё — плита с четырьмя линзами-квадрантами, бликом и светящейся кромкой.",
                           "The compass has moved onto a glass plate: the stack of offset layers behind the board is gone, replaced by a plate with four quadrant lenses, a sheen and a glowing rim."],
    "wel.up.l2":          ["Траектории партий переписаны заново. Раньше короткие маршруты растягивались веером — то есть рисовались не там, где партия была на самом деле; теперь все узлы стоят на своих числах.",
                           "Party trajectories have been rewritten. Short routes used to be fanned out — drawn where the party never actually was; now every node sits on its real numbers."],
    "wel.up.l3":          ["Направление движения партии читается толщиной линии, а не стрелкой в конце, и годы больше не висят поверх поля — они в подсказке.",
                           "A party's direction of travel now reads from the thickness of the line rather than an arrowhead, and the years no longer hang over the board — they live in the tooltip."],
    "wel.up.l4":          ["Пунктир от точки до осей красится цветом самой точки и показывает обе координаты числами прямо на осях.",
                           "The dashed guides from a dot to the axes now take the dot's own colour and show both coordinates as numbers right on the axes."],
    "wel.up.l5":          ["Подписи партий больше не отъезжают от своих точек: раскладчик считал точку препятствием для её собственного имени. Заметнее всего это было на ППД.",
                           "Party labels no longer drift away from their dots: the placer counted a dot as an obstacle for its own name. It showed up worst on the Direct Democracy Party."],
    ...
    "foot.version.tip":   ["Версия 2.4.1: компас переехал на стеклянную подложку, траектории партий переписаны заново, перекрестие показывает координаты цветом точки, подписи больше не отъезжают от своих точек",
                           "Version 2.4.1: the compass has moved onto a glass plate, party trajectories have been rewritten from scratch, the crosshair now shows the coordinates in the dot's own colour, and labels no longer drift away from their dots"],

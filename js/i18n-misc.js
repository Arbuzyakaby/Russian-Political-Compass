...
(function(PC){
  ...
  Object.assign(PC.i18nDict, {
    ...
    "wel.up.l1":          ["Настройки: переключение разделов теперь анимировано, а не мгновенная подмена.",
                           "Settings: switching between sections is now animated instead of an instant swap."],
    "wel.up.l2":          ["Настройки: окно держит одну высоту на всех разделах, а прокрутка возвращается к началу при смене раздела.",
                           "Settings: the window keeps one fixed height across sections, and switching a section now resets its scroll to the top."],
    "wel.up.l3":          ["Новый раздел «Приватность» (PVFP) — на одном экране собрано всё, что раньше было разбросано по подвалу.",
                           "A new Privacy section (PVFP) gathers on one screen what used to be scattered across the footer."],
    "wel.up.l4":          ["Состав палаты: 450 мест дуги проявляются волной слева направо при первой отрисовке.",
                           "Chamber composition: all 450 seats of the arc now appear in a left-to-right wave on first render."],
    "wel.up.l5":          ["Легенда состава палаты получила заливку под строкой — доля фракции видна до того, как прочитан процент.",
                           "The chamber-composition legend now shows a fill behind each row — a faction's share is visible before you read the percentage."],
    ...
    "foot.version.tip":   ["Версия 2.3.1.1: анимация переключения разделов настроек, окно настроек одной высоты, раздел «Приватность» (PVFP), редизайн состава палаты",
                           "Version 2.3.1.1: animated settings-panel switching, a settings window with one fixed height, a Privacy section (PVFP), a redesigned chamber-composition chart"],

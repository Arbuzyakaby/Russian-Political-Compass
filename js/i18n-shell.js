...
(function(PC){
  ...
  Object.assign(PC.i18nDict, {
    /* ===== оболочка страницы ===== */
    "doc.title.compass":  ["Русский политический компас — партии РФ на двух осях", "Russian Political Compass"],
    "doc.title.quiz":     ["Тест: где вы на компасе — Русский политический компас",
                           "Quiz: where do you stand — Russian Political Compass"],
    "doc.title.votes":    ["Ключевые голосования Госдумы — Русский политический компас",
                           "Landmark Duma votes — Russian Political Compass"],
    "doc.title.about":    ["О проекте и методике — Русский политический компас",
                           "About the method — Russian Political Compass"],
    "doc.desc":           ["Русский политический компас: партии России на двух осях — экономика и отношение к власти государства. Мандаты в Госдуме по созывам IV–VIII, ключевые голосования и тест на 90 утверждений в трёх версиях.",
    ...
    /* ===== шапка ===== */
    "brand.eyebrow":      ["Русский политический компас · Политология", "Russian Political Compass · Political science"],
    ...
    "set.theme.alaska":   ["Аляска 2.0", "Alaska 2.0"],
    ...
    /* ===== SRPC: раздел настроек и полноэкранный режим (2.4.2.1) ===== */
    "set.srpc":           ["SRPC", "SRPC"],
    "set.srpcfail":       ["Демонстрация сбоя", "Failure demo"],
    "set.srpcfail.d":     ["Третье звено схемы в «О проекте» уходит в ошибку и гасит всё, что от него зависит. Выключено по умолчанию: без предупреждения это читается как поломка, а не как рассказ",
                           "The third link of the diagram in the About tab fails and takes everything downstream with it. Off by default: with no warning it reads as a malfunction rather than as a story"],
    "srpc.badge":         ["Собственный протокол проекта", "The project’s own protocol"],
    "srpc.title":         ["SRPC — Static Render Pipeline Chain", "SRPC — Static Render Pipeline Chain"],
    "srpc.lead":          ["Страница собирается конвейером из шести этапов: разметка, стили, данные, расчёты, отрисовка, экспорт. Ни сборщика, ни сервера, ни единого запроса наружу — весь путь от файла до картинки проходит в вашем браузере. Схема этого пути стоит во вкладке «О проекте», а здесь её можно разложить на столе и рассмотреть каждое звено отдельно.",
                           "The page is assembled by a six-stage pipeline: markup, styles, data, maths, drawing, export. No bundler, no server, not a single outbound request — the whole way from file to picture happens in your browser. The diagram of that way lives in the About tab; here you can lay it out on a table and look at every link on its own."],
    "srpc.open":          ["Открыть во весь экран", "Open full screen"],
    "srpc.full.badge":    ["SRPC · рабочий стол", "SRPC · workbench"],
    "srpc.full.hint":     ["Карточки перетаскиваются мышью, пальцем и стрелками с клавиатуры — связи следуют за ними, а раскладка запоминается. Цикл идёт полностью: сборка, сбой в третьем звене, каскад и восстановление.",
                           "Drag the cards with a mouse, a finger or the arrow keys — the links follow them and the layout is remembered. The cycle runs in full: assembly, failure in the third link, the cascade and the recovery."],
    "srpc.replay":        ["Проиграть заново", "Play again"],
    "srpc.reset":         ["Расставить как было", "Reset the layout"],
    "srpc.reset.done":    ["Карточки вернулись на исходные места", "Cards are back where they started"],
    "srpc.close":         ["Закрыть", "Close"],
    "srpc.keys":          ["Esc — закрыть · стрелки — двигать карточку · Shift + стрелки — быстрее",
                           "Esc — close · arrows — move a card · Shift + arrows — faster"],
    "srpc.d1":            ["Один документ, в котором лежит вся страница целиком: четыре вкладки, панель настроек, приветствие и подвал. Ничего не подгружается по дороге — то, что пришло первым запросом, и есть весь сайт.",
                           "A single document holding the entire page: four tabs, the settings panel, the welcome screen and the footer. Nothing is fetched along the way — what arrived with the first request is the whole site."],
    "srpc.d2":            ["Два десятка файлов стилей, собранных вокруг одного набора токенов: семь тем и полтора десятка настроек интерфейса меняются подменой значений, а не переписыванием правил.",
                           "Two dozen stylesheets built around one set of tokens: seven themes and a dozen-plus interface settings work by swapping values, not by rewriting rules."],
    "srpc.d3":            ["Партии, созывы, голосования и утверждения теста — обычные массивы в обычных файлах. Ни базы, ни запроса к серверу: этот этап и есть тот вход, без которого дальше считать нечего.",
                           "Parties, convocations, votes and quiz statements — plain arrays in plain files. No database, no server call: this stage is the input without which nothing downstream has anything to compute."],
    "srpc.d4":            ["Координаты, расстояния между партиями, состав палаты, совпадения фракций и результат теста. Всё арифметика на этих же данных, и всю её можно проверить, открыв один файл.",
                           "Coordinates, distances between parties, the makeup of the chamber, faction agreement and the quiz result. All of it is arithmetic over the same data, and all of it can be checked by opening one file."],
    "srpc.d5":            ["Компас, парламентская диаграмма, динамика, спектр и паук рисуются в SVG прямо в браузере — узлами документа, а не картинками. Поэтому они остаются чёткими на любом экране и в любом масштабе.",
                           "The compass, the hemicycle, the trend, the spectrum and the radar are drawn as SVG right in the browser — as document nodes, not images. That is why they stay sharp on any screen and at any zoom."],
    "srpc.d6":            ["PNG, SVG, CSV и JSON собираются здесь же, из уже нарисованного. Файл рождается в браузере и в браузере остаётся: сервер не участвует в сборке и не видит, что вы выгрузили.",
                           "PNG, SVG, CSV and JSON are assembled right here, from what has already been drawn. The file is born in the browser and stays there: no server takes part and none sees what you exported."],

    /* ===== обучение по живой странице (2.4.2.1) ===== */
    "set.tour":           ["Обучение", "Guided tour"],
    "set.tour.d":         ["Десять шагов по самой странице: экран затемняется, а подсвеченным остаётся тот элемент, о котором идёт речь",
                           "Ten steps across the page itself: the screen dims and the element being explained stays lit"],
    "set.tour.start":     ["Пройти обучение", "Take the tour"],
    "tour.count":         ["Шаг {n} из {total}", "Step {n} of {total}"],
    "tour.next":          ["Дальше", "Next"],
    "tour.back":          ["Назад", "Back"],
    "tour.skip":          ["Пропустить", "Skip"],
    "tour.done":          ["Всё понятно", "Got it"],
    "tour.tabs.h":        ["Четыре раздела", "Four sections"],
    "tour.tabs.p":        ["Компас с аналитикой, тест, ключевые голосования Госдумы и раздел о методике. Переключаются здесь и ничего не перезагружают: адрес меняется, страница остаётся той же.",
                           "The compass with its analytics, the quiz, the landmark Duma votes and the section on method. They switch here and reload nothing: the address changes, the page stays the same."],
    "tour.board.h":       ["Поле из двух осей", "A field with two axes"],
    "tour.board.p":       ["По горизонтали — экономика: слева плановая, справа рыночная. По вертикали — сколько власти отдано государству. Обе шкалы идут от −10 до +10 и независимы друг от друга.",
                           "Horizontally, the economy: planned on the left, market on the right. Vertically, how much power goes to the state. Both scales run from −10 to +10 and are independent of each other."],
    "tour.node.h":        ["Каждая точка — партия", "Every dot is a party"],
    "tour.node.p":        ["Размер кружка — мандаты в выбранном созыве. Наведите курсор, чтобы увидеть короткую справку, нажмите — откроется карточка с программой и обоснованием обеих координат.",
                           "The size of the dot is its seats in the selected convocation. Hover for a short summary, click to open the card with the platform and the reasoning behind both coordinates."],
    "tour.conv.h":        ["Созыв Госдумы", "Duma convocation"],
    "tour.conv.p":        ["Пять созывов с 2003 года. Переключение меняет не только размеры точек, но и все графики ниже: состав палаты, динамику мандатов и спектр.",
                           "Five convocations since 2003. Switching changes not only the size of the dots but every chart below: the makeup of the chamber, the seat trend and the spectrum."],
    "tour.search.h":      ["Поиск и фильтры", "Search and filters"],
    "tour.search.p":      ["Поиск понимает оба языка: «КПРФ» и «CPRF» найдут одно и то же. Чипы рядом с полем оставляют на поле только думские партии или только внедумские.",
                           "Search understands both languages: the Russian and the English name of a party find the same dot. The chips next to the field leave only parliamentary or only non-parliamentary parties on the board."],
    "tour.side.h":        ["Панель справа", "The panel on the right"],
    "tour.side.p":        ["Пока партия не выбрана, здесь список всех точек поля. Выберите одну — панель перевернётся и покажет карточку: программа, мандаты по созывам, под-оси и разбор координат.",
                           "Until a party is selected this holds the list of every dot on the board. Pick one and the panel flips to its card: the platform, seats by convocation, sub-axes and the reasoning behind the coordinates."],
    "tour.trails.h":      ["Траектории во времени", "Trails over time"],
    "tour.trails.p":      ["Показывает, как партия двигалась по полю от созыва к созыву. Линия толще ближе к настоящему — по этому и читается направление, стрелки не нужны.",
                           "Shows how a party moved across the board from one convocation to the next. The line thickens towards the present — that is how direction reads, no arrowheads needed."],
    "tour.export.h":      ["Выгрузка", "Export"],
    "tour.export.p":      ["Поле в PNG или SVG, данные в CSV или JSON. Файл собирается прямо в браузере и никуда не отправляется — сервер не участвует и не знает, что вы выгрузили.",
                           "The board as PNG or SVG, the data as CSV or JSON. The file is assembled right in the browser and goes nowhere — no server takes part and none knows what you exported."],
    "tour.quiz.h":        ["Найдите себя на поле", "Find yourself on the board"],
    "tour.quiz.p":        ["Тест на 20, 40 или 90 утверждений ставит вашу точку рядом с партиями и разбирает каждый ответ. Ответы остаются в вашем браузере и никуда не уходят.",
                           "A quiz of 20, 40 or 90 statements puts your own dot next to the parties and breaks down every answer. The answers stay in your browser and go nowhere."],
    "tour.settings.h":    ["Настройки", "Settings"],
    "tour.settings.p":    ["Семь тем, размер текста, плотность вёрстки, анимации, шрифт и ещё полтора десятка переключателей. Там же можно запустить это обучение заново.",
                           "Seven themes, text size, layout density, animations, the typeface and a dozen more switches. The same place can start this tour again."],
